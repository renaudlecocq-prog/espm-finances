      ...
      <div className="flex-1 overflow-y-auto p-6">
        <div className="max-w-xl mx-auto w-full space-y-4">
          ...
          {tab === 'securite' && (
            <div className={card}>
              ...
              <form onSubmit={handlePasswordSubmit} className="space-y-4">
                {hasPassword && (
                  <div>
                    <label className="text-xs font-medium text-gray-600 block mb-1">Ancien mot de passe</label>
                ...
                <div>
                  ...
                  <div className="relative">
                    <input type={showPwd ? 'text' : 'password'} value={newPwd} onChange={e => setNewPwd(e.target.value)} required minLength={8}
                      className="w-full border border-gray-200 rounded-lg px-3 py-2 pr-10 text-sm focus:outline-none focus:border-indigo-400" />
                    <button type="button" onClick={() => setShowPwd(v => !v)}
                      className="absolute right-3 top-1/2 -translate-y-1/2 text-gray-400 hover:text-gray-600">
                      {showPwd ? <EyeOff size={14} /> : <Eye size={14} />}
                    </button>
                  </div>
                  <p className="text-[10px] text-gray-400 mt-1">Minimum 8 caractères</p>
                </div>
                <div>
                ...
                {pwdMsg && (
                  <div className={`flex items-center gap-2 text-sm px-3 py-2.5 rounded-lg ${pwdMsg.ok ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
          ...
          {tab === 'notifications' && (
            <div className={card}>
              <div className="flex items-center gap-3 mb-6">
                ...
                <div>
                  <h2 className="font-semibold text-gray-800">Plages silencieuses</h2>
                  <p className="text-xs text-gray-500">Badge et alertes temps réel coupés pendant les plages définies</p>
                </div>
              </div>

              {/* Toggle */}
              <div className="flex items-center justify-between p-4 bg-gray-50 rounded-xl mb-5">
                <span className="text-sm font-medium text-gray-700">Activer les plages silencieuses</span>
                <button onClick={() => setSchedule(s => ({ ...s, enabled: !s.enabled }))}
                  ...
                  <div className={`absolute top-0.5 w-5 h-5 bg-white rounded-full shadow transition-transform ${schedule.enabled ? 'translate-x-5' : 'translate-x-0.5'}`} />
                </button>
              </div>
              ...
              {schedule.enabled && (
                <div className="space-y-5">
                  <div className="grid grid-cols-2 gap-4">
                    <div>
                      <label className="text-xs font-medium text-gray-600 block mb-1.5">Silence à partir de</label>
                    ...
                    <div>
                      <label className="text-xs font-medium text-gray-600 block mb-1.5">Silence jusqu'à</label>
                  ...
                  <div>
                    <label className="text-xs font-medium text-gray-600 block mb-2">Jours silencieux</label>
                    <div className="flex gap-1.5 flex-wrap">
                      {DAYS.map((d, i) => (
                        <button key={i} onClick={() => toggleDay(i)}
                          className={`w-9 h-9 rounded-full text-xs font-semibold transition-colors
                            ${schedule.muteDays.includes(i) ? 'bg-indigo-600 text-white' : 'bg-gray-100 text-gray-600 hover:bg-gray-200'}`}>
                          {d}
                        </button>
                      ))}
                    </div>
              ...
              {notifMsg && (
                <div className={`flex items-center gap-2 text-sm px-3 py-2.5 rounded-lg mt-4 ${notifMsg.ok ? 'bg-green-50 text-green-700 border border-green-100' : 'bg-red-50 text-red-600 border border-red-100'}`}>
          ...
          {tab === 'preferences' && (
            <>
              {[
                { icon: <Palette size={18} className="text-purple-500" />, bg: 'bg-purple-50', title: 'Thème', desc: "Choisissez le thème visuel de l'application" },
                { icon: <Home     size={18} className="text-blue-500"   />, bg: 'bg-blue-50',   title: "Page d'accueil", desc: 'Personnalisez les widgets de votre tableau de bord' },
              ].map(item => (
                <div key={item.title} className={`${card} flex items-center gap-4`}>
                  ...
                  <span className="text-xs font-semibold bg-amber-100 text-amber-700 px-2.5 py-1 rounded-full shrink-0">À venir</span>
                </div>
              ))}
            </>
          )}
