...
// ── Sélecteur avec recherche + chips ─────────────────────────────────────────
function MultiSelect({ label, all, selected, onToggle, color = 'blue' }) {
  ...
  const accent = color === 'purple'
    ? { chip: 'bg-purple-500 text-white', ring: 'ring-purple-400', dot: 'bg-purple-400' }
    : { chip: 'bg-blue-500 text-white',   ring: 'ring-blue-400',   dot: 'bg-blue-400'   }
    ...
    <div className="mb-4" ref={ref}>
      ...
      {selected.length > 0 && (
        <div className="flex flex-wrap gap-1 mb-2">
          {selected.map(v => (
            <span key={v} className={`inline-flex items-center gap-1 px-2 py-0.5 rounded-lg text-xs font-medium ${accent.chip}`}>
              {v}
              <button
                onClick={() => onToggle(v)}
      ...
      <div className="relative">
        <input
          ...
          className={`w-full text-xs px-3 py-1.5 rounded-lg border border-gray-200 dark:border-gray-600 bg-white dark:bg-gray-800 text-gray-700 dark:text-gray-200 placeholder-gray-400 outline-none focus:ring-2 ${accent.ring}`}
        />
  ...
  return (
    ...
    >
      <div className="flex items-center justify-between px-4 py-2.5 bg-gray-50 dark:bg-gray-800 border-b border-gray-200 dark:border-gray-700">
        ...
        <div className="flex items-center gap-2">
          <button onClick={fetchEleves} title="Actualiser" className="text-gray-400 hover:text-gray-600 dark:hover:text-gray-300 transition-colors">
            <svg className={`w-3.5 h-3.5 ${loading ? 'animate-spin' : ''}`} fill="none" viewBox="0 0 24 24" stroke="currentColor">
              <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M4 4v5h.582m15.356 2A8.001 8.001 0 004.582 9m0 0H9m11 11v-5h-.581m0 0a8.003 8.003 0 01-15.357-2m15.357 2H15" />
            </svg>
          </button>
          {editable && (
            <button onClick={onEdit} className="flex items-center gap-1 text-xs text-gray-400 hover:text-blue-500 dark:hover:text-blue-400 transition-colors">
